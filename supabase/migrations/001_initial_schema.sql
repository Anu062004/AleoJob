...
-- Create index on aleo_address for faster lookups
...
CREATE TABLE jobs (
  ...
  zk_membership_hash TEXT NOT NULL, -- Proof that payer paid 3 Aleo
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);
...
CREATE TABLE applications (
  ...
  zk_application_hash TEXT NOT NULL, -- Proof that seeker paid 1 Aleo
...
-- Reputation Events Table (Optional but Powerful)
CREATE TABLE reputation_events (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES users(id) ON DELETE CASCADE NOT NULL,
  delta INT NOT NULL, -- Positive or negative reputation change
  reason TEXT,
  zk_proof_hash TEXT,
  created_at TIMESTAMP DEFAULT NOW()
);

-- Create indexes for reputation events
