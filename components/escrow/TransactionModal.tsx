  ...
  return (
    <Modal isOpen={open} onClose={onClose} title={getTitle()}>
      <div className="space-y-4">
        ...
        <p className="text-center text-text-secondary">{getMessage()}</p>

        {transactionId && (
          <div className="bg-background-alt rounded-xl p-4 border border-border-light">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-sm text-text-secondary mb-1">Transaction ID</p>
                <p className="text-xs font-mono text-text-dark truncate">{transactionId}</p>
              </div>
              <a
                href={explorerUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="ml-4 text-primary hover:text-primary-hover"
              >
