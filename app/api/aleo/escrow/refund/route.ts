// API Route: Refund Escrow Payment
// POST /api/aleo/escrow/refund
// POST /api/escrows/:id/refund (alternative route pattern)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { escrowId, employerPrivateKey, aleoAddress, refundReason = 0 } = body;

    // Validation
    if (!escrowId || !employerPrivateKey || !aleoAddress) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: escrowId, employerPrivateKey, aleoAddress' },
        { status: 400 }
      );
    }
