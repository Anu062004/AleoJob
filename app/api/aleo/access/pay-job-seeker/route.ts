// API Route: Pay for Job Seeker Access
// POST /api/aleo/access/pay-job-seeker
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Execute transition on Aleo blockchain (deducts 1 credit)
    const transaction = await aleoService.payJobSeekerAccess(recipient, keyToUse);
    ...
    return NextResponse.json({
      ...
      message: 'Job seeker access payment submitted',
    });
  } catch (error: any) {
    return NextResponse.json(
